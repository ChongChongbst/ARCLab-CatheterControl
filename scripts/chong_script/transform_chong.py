...
class FindCurvature:
    ...
    def calculate_bezier_specs(self):
            ...
            self.bezier_specs = np.zeros((4, 3))
            ...
            self.bezier_specs[0, :] = p_0
            self.bezier_specs[1, :] = p_end
            self.bezier_specs[2, :] = c1
            self.bezier_specs[3, :] = c2
    ...
    def get_points_on_curve(self, N):
        ...
        self.p3d_list = np.zeros((N,3))
        ts = np.linspace(0, 1, num=N)

        for i in range(N):
            t = ts[i]
            self.p3d_list[i] = c + (1-t)*(1-t)*(p_0-c) + t*t*(p_end-c)

        #self.p3d_list = self.p3d_list[10:]
        return self.p3d_list



    def get_points_on_curve_5000(self, bezier_specs):
        '''
        get 5000 3D points on the quadratic bezier curve 

        Args:
            bezier_specs((4,3) numpy array): the cubic bezier specs
        '''
        p_0 = bezier_specs[0, :]
        p_end = bezier_specs[1, :]
        c1 = bezier_specs[2, :]
        c = (3*c1 -p_0)/2

        p3d_list = np.zeros((5000,3))
        ts = np.linspace(0, 1, num=5000)

        for i in range(5000):
            t = ts[i]
            p3d_list[i] = c + (1-t)*(1-t)*(p_0-c) + t*t*(p_end-c)
        
        p3d = p3d_list[4990:5010]
    ...
    def project_3d_camera_matrix(self, p3d_list, camera_matrix):
        '''
        ...
        for i in range(length):
            ...
            p2d = camera_matrix @ p3d
            p2d_list[i] = np.true_divide(p2d[:2], p2d[-1])
        return p2d_list



    def calculate_2d_curve_length(self, bezier_specs):
        """
        ...
        Args:
            bezier_specs((4,3) numpy array): the cubic bezier specs
        """
        p3d_5000 = self.get_points_on_curve_5000(bezier_specs)
    ...
    def get_TNB_curvature(self, pd_list, l):
        """
        Use TNB function to get curvature
    ...
    def curvature_change(self, N, method, p2d_list=0):
        """
        ...
        for i in range(length-2)[1:]:
            ...
            if method == 1:
                ...
                if (i-N) < 0:
                    p3d_3[0] = self.p3d_list[0]
                else:
                    p3d_3[0] = self.p3d_list[i-N]

                p3d_3[1] = self.p3d_list[i]

                if (i+N) > (length-1):
                    p3d_3[2] = self.p3d_list[length-1]
                else:
                    p3d_3[2] = self.p3d_list[i+N]
                ...
                if (i-N) < 0:
                    ...
                    p2d_3[0] = p2d_list[i-N]

                p2d_3[1] = p2d_list[i]

                if (i+N) > (length-1):
                    p2d_3[2] = p2d_list[length-1]
                else:
                    p2d_3[2] = p2d_list[i+N]
    ...
    def is_points_in_image(self, p_proj, width, height):
        '''
    ...
    def test(self):
        '''
        the test code for Chong
        '''
        path = "/home/catheter/Documents/ARCLab-CCCatheter/test_chong/"
        img = cv2.imread(path + "blenderpic1.png")

        p3d = np.load(path + "p3d_poses.npy")
        ...
        bezier_specs = calculate_bezier_specs(cc_pt_list)
        print("Cubic bezier specs are:")
        print(bezier_specs)
        p3d_list = get_points_on_curve_1000(bezier_specs)
        p2d_list = project_3d_to_2d(p3d_list)
        p2d_list_cm = project_3d_camera_matrix(p3d_list, camera_matrix)
        ...
        cv2.imwrite(path+"proj_1.png", img)
        ...
        k1 = curvature_change(p3d_list[10:], 10, 1)
        ...
        k2 = curvature_change(p3d_list[10:], 10, 2)
